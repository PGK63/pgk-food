...
class AdminRepository {
    suspend fun getFraudReports(token: String, startDate: String, endDate: String): Result<List<FraudReportDto>> = runCatching {
    ...
    suspend fun resolveFraud(token: String, id: Int): Result<Unit> = runCatching {
        SharedNetworkModule.client.post(SharedNetworkModule.getUrl("/api/v1/reports/fraud/$id/resolve")) {
            header(HttpHeaders.Authorization, "Bearer $token")
        }
    }

    suspend fun getDailyReport(token: String, date: String): Result<DailyReportDto> = runCatching {
    ...
    suspend fun getWeeklyReport(token: String, startDate: String): Result<List<DailyReportDto>> = runCatching {
    ...
    suspend fun exportReportsCsv(token: String, startDate: String, endDate: String): Result<ByteArray> = runCatching {
    ...
    suspend fun exportReportsPdf(token: String, startDate: String, endDate: String): Result<ByteArray> = runCatching {
