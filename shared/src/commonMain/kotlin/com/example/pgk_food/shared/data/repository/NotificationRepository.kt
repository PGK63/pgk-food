...
class NotificationRepository {
    suspend fun getNotifications(
        ...
        return safeApiCall {
            SharedNetworkModule.client.get(SharedNetworkModule.getUrl("/api/v1/notifications")) {
    ...
    suspend fun getUnreadCount(token: String): ApiResult<UnreadCountDto> {
        return safeApiCall {
            SharedNetworkModule.client.get(SharedNetworkModule.getUrl("/api/v1/notifications/unread-count")) {
                header(HttpHeaders.Authorization, "Bearer $token")
            }.body()
        }
    }
    ...
    suspend fun markAsRead(token: String, id: Long): ApiResult<Unit> {
        return safeApiCall {
            SharedNetworkModule.client.post(SharedNetworkModule.getUrl("/api/v1/notifications/$id/read")) {
                header(HttpHeaders.Authorization, "Bearer $token")
            }
            Unit
        }
    ...
    suspend fun markAsReadBatch(token: String, ids: List<Long>): ApiResult<Unit> {
        return safeApiCall {
            SharedNetworkModule.client.post(SharedNetworkModule.getUrl("/api/v1/notifications/read-batch")) {
                header(HttpHeaders.Authorization, "Bearer $token")
