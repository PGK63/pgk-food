...
import com.example.pgk_food.util.MenuCsvParser
...
fun ChefMenuManageScreenV2(
    ...
    var copyDate by remember { mutableStateOf(LocalDate.now().plusDays(1)) }
    ...
    suspend fun importCsv(uri: Uri) {
        val fileBytes = context.contentResolver.openInputStream(uri)?.use { it.readBytes() }
        if (fileBytes == null) {
            snackbarHostState.showSnackbar("Не удалось прочитать CSV файл")
            return
        }
            ...
            if (batchResult.isSuccess) {
                ...
                requests.forEachIndexed { index, request ->
                    chefRepository.addMenuItem(token, request)
                        ...
                        .onFailure { sendErrors += "Строка ${index + 1}: ${it.userMessage}" }
                }
            }
        }
                ...
                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = 0.4f))) {
                    Column(modifier = Modifier.padding(12.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            Button(onClick = { csvPickerLauncher.launch(arrayOf("text/*")) }) {
            ...
            text = {
                Column {
                    ...
                    Row(modifier = Modifier.fillMaxWidth().clickable { showCopyDatePicker = true }) {
                        Icon(Icons.Rounded.CalendarMonth, contentDescription = null)
                        Spacer(modifier = Modifier.width(8.dp))
                        Text(copyDate.format(formatter))
                    }
                }
            },
            confirmButton = {
    ...
    AlertDialog(
        ...
        text = {
            Column {
                ...
                Spacer(modifier = Modifier.height(8.dp))
                TextButton(onClick = { datePickerVisible = true }) { Text("Дата: $date") }
