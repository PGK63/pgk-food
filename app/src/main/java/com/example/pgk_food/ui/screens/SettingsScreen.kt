...
import com.example.pgk_food.data.remote.dto.NotificationDto
...
fun SettingsScreen(
    ...
    LaunchedEffect(token, isCurator) {
        NotificationAutoRefreshBus.events.collect {
            loadNotifications()
        }
    }
                    ...
                    Card(
                        ...
                        Column(modifier = Modifier.padding(14.dp)) {
                            ...
                            Row(
                                ...
                                Switch(
                                    checked = showHints,
                                    onCheckedChange = { updateHints(it) }
                                )
                            }
                    ...
                    Card(
                        ...
                        Column(modifier = Modifier.padding(14.dp)) {
                            ...
                            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                Button(onClick = { scope.launch { loadNotifications() } }, enabled = !isLoading) {
                                    Text("Обновить")
                                }
                                Button(
                                    onClick = { markAllRead() },
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            if (rosterDeadline.needsReminder) {
                Text(
                    text = "Нужно заполнить табель на следующую неделю. До дедлайна: ${rosterDeadline.daysUntilDeadline ?: "?"} дн.",
                    style = MaterialTheme.typography.bodySmall
                )
                rosterDeadline.deadlineDate?.let {
                    Text("Дедлайн: $it", style = MaterialTheme.typography.bodySmall)
                }
            } else {
                Text(
                    text = rosterDeadline.reason ?: "Напоминание не требуется.",
                    style = MaterialTheme.typography.bodySmall
                )
            }
        }
