...
- Если есть невалидные строки, запускает интерактивный редактор в терминале (`Редактировать / Пропустить / Прервать`).
- После интерактивной сессии сохраняет исправленную копию исходника рядом: `*_fixed_<timestamp>.xlsx`.
- Baseline preflight для сценария с одним системным админом:
  - игнорирует текущего админа (под которым выполнен вход в утилите);
  - игнорирует его служебную группу;
  - любые другие группы/кураторы/студенты блокируют импорт.
...
Интерактивная правка проблемных строк запускается автоматически в обоих режимах (`dry-run` и `apply`), если найдены ошибки.
...
- `report.json` — итоговый отчет (статусы, счетчики, preflight baseline, интерактивная правка, rollback).
- `reject.csv` — отклоненные строки с причиной.
- `credentials.xlsx` — учетные данные:
  - лист `Кураторы`;
  - лист `Студенты`.

Рядом с исходным `xlsx` при интерактивной правке создается:

- `*_fixed_<timestamp>.xlsx` — исправленная копия входного Excel.
...
- Для интерактивного режима нужен TTY (обычный запуск в терминале).
